...
public class AuthService {
     ...
     */
    public UsuarioDTO register(RegistroDto registroDto, HttpServletRequest request) {
        if (registroDto.getNombre() == null || registroDto.getNombre().trim().length() < 2) {
            throw new RuntimeException("El nombre completo debe tener al menos 2 caracteres");
        }
        if (!registroDto.getNombre().trim().matches("^[a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\\s]+$")) {
            throw new RuntimeException("El nombre completo solo puede contener letras y espacios");
        }
        ...
        // Autenticar la sesión automáticamente para el usuario recién registrado
    ...
    private UsuarioDTO mapToDto(Usuario usuario) {
        String estadoRefugio = refugioRepository.findByIdUsuario(usuario.getId())
                .map(com.pawtok.model.Refugio::getEstadoVerificacion)
                .orElse(null);
