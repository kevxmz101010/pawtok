...
public class MascotaService {
    ...
    public org.springframework.data.domain.Page<MascotaDTO> getMascotasByUsuarioEmail(String email, org.springframework.data.domain.Pageable pageable) {
        ...
        if (usuario.getRol() != Rol.REFUGIO && usuario.getRol() != Rol.ADMIN) {
            throw new org.springframework.web.server.ResponseStatusException(
                org.springframework.http.HttpStatus.FORBIDDEN, "Solo los refugios y administradores tienen publicaciones de mascotas"
            );
        }
    ...
    public MascotaDTO createMascota(MascotaDTO mascotaDto, String refugioEmail) {
        ...
        if (usuario.getRol() != Rol.REFUGIO && usuario.getRol() != Rol.ADMIN) {
            throw new org.springframework.web.server.ResponseStatusException(
                ...
                "Solo los refugios verificados y administradores pueden publicar mascotas"
            );
        }

        com.pawtok.model.Refugio refugio = refugioRepository.findByIdUsuario(usuario.getId())
                .orElseThrow(() -> new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.NOT_FOUND, "Refugio no encontrado para este usuario"
                ));

        if (usuario.getRol() != Rol.ADMIN && !"Aprobado".equalsIgnoreCase(refugio.getEstadoVerificacion())) {
            throw new org.springframework.web.server.ResponseStatusException(
                org.springframework.http.HttpStatus.FORBIDDEN, 
                "Tu refugio se encuentra en proceso de verificación. Debe ser aprobado por un administrador antes de publicar mascotas."
            );
        }
        ...
        guardarImagenesMascota(saved.getId(), saved.getImagenUrl(), saved.getGaleria());
    ...
    public void deleteMascota(Long id, String usuarioEmail) {
        ...
        try {
            mascotaImagenRepository.deleteByIdMascota(mascota.getId());
        } catch (Exception e) {
            // Cascade en MySQL maneja la eliminación de imágenes en caso de fallo
        }
    ...
    /**
     * Sincroniza las imágenes de una mascota en la tabla mascota_imagenes.
     * Guarda la imagen principal con orden_img = 0 y la galería con orden_img = 1, 2, ...
     */
    private void guardarImagenesMascota(Long idMascota, String imagenPrincipal, String galeriaStr) {
        if (idMascota == null) return;
        try {
            mascotaImagenRepository.deleteByIdMascota(idMascota);
            if (imagenPrincipal != null && !imagenPrincipal.isBlank()) {
                mascotaImagenRepository.save(com.pawtok.model.MascotaImagen.builder()
                        .idMascota(idMascota)
                        .nombreArchivo(imagenPrincipal)
                        .ordenImg(0)
                        .fechaSubida(java.time.LocalDateTime.now())
                        .activo(true)
                        .build());
            }
            if (galeriaStr != null && !galeriaStr.isBlank()) {
                String[] arr = galeriaStr.split(",");
                int order = 1;
                for (String img : arr) {
            ...
            System.err.println("Error sincronizando mascota_imagenes: " + e.getMessage());
        }
    }
    ...
    private MascotaDTO mapToDto(Mascota mascota) {
        ...
        try {
            List<com.pawtok.model.MascotaImagen> imagenes = mascotaImagenRepository
                    .findByIdMascotaAndActivoTrueOrderByOrdenImgAsc(mascota.getId());
            if (imagenes != null && !imagenes.isEmpty()) {
                List<String> galeriaFromDb = new java.util.ArrayList<>();
                for (com.pawtok.model.MascotaImagen img : imagenes) {
                    if (img.getOrdenImg() != null && img.getOrdenImg() == 0) {
                        imagenPrincipal = img.getNombreArchivo();
                    } else {
                        galeriaFromDb.add(img.getNombreArchivo());
                    }
                }
                if (!galeriaFromDb.isEmpty()) {
                    galeriaList = galeriaFromDb;
                }
            }
        } catch (Exception e) {
            // Fallback a columnas de la entidad Mascota
        }

        if (galeriaList == null && mascota.getGaleria() != null && !mascota.getGaleria().isEmpty()) {
            galeriaList = java.util.Arrays.asList(mascota.getGaleria().split(","));
        }

        String refugioNombre = "Refugio Pawtok";
        String refugioHorario = null;
        ...
        if (idRefugio == null && mascota.getRefugio() != null && !mascota.getRefugio().isEmpty()) {
            try {
                idRefugio = Long.parseLong(mascota.getRefugio());
