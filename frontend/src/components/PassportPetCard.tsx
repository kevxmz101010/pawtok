...
import { formatCleanText } from '../utils/imageUtils';
  ...
  return (
    <div className="w-full h-[340px] select-none relative" style={{ perspective: '1000px' }}>
      ...
      >
        ...
        {pet.matchScore !== undefined && pet.matchScore > 0 && (
          <div
            className="absolute top-4 left-4 z-30 pointer-events-none"
            style={{ transform: 'translateZ(25px)' }}
          >
            {pet.matchScore >= 85 ? (
              <div className="px-3 py-1.5 bg-gradient-to-r from-emerald-600/5 to-green-500/30 backdrop-blur-md border border-white/30 text-white text-xs sm:text-sm font-medium rounded-full flex items-center gap-1.5">
                <span>{pet.matchScore}% Match Ideal</span>
              </div>
            ) : pet.matchScore >= 70 ? (
              <div className="px-3 py-1.5 bg-gradient-to-r from-blue-600/95 to-cyan-500/95 backdrop-blur-md border border-white/30 text-white text-xs sm:text-sm font-medium rounded-full shadow-[0_4px_14px_rgba(14,165,233,0.4)] flex items-center gap-1.5">
                <Check size={14} className="text-cyan-100 shrink-0" />
                <span>{pet.matchScore}% Compatible</span>
              </div>
            ) : (
              <div className="px-3 py-1.5 bg-gray-800/85 backdrop-blur-md border border-white/20 text-gray-200 text-xs font-semibold rounded-full shadow flex items-center gap-1.5">
                <Shield size={13} className="text-gray-300 shrink-0" />
                <span>{pet.matchScore}% Afinidad</span>
              </div>
            )}
          </div>
        )}
        ...
        >
          <h3 className="text-[22px] font-semibold text-white leading-tight truncate drop-shadow-md group-hover:text-[#57abff] transition-colors">
            {formatCleanText(pet.nombre)}
          </h3>
          <p className="text-[14px] text-gray-300 font-medium drop-shadow-sm flex items-center gap-2">
            <span>{formatCleanText(pet.raza || pet.categoria)}</span>
            {pet.tamano && <span className="text-gray-300 text-xs">• {formatCleanText(pet.tamano)}</span>}
          </p>
          {pet.matchReason && (
            <div className="mt-1">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-white/20 backdrop-blur-md border border-white/25 text-[11px] font-medium text-white rounded-lg shadow-sm">
                <Sparkles size={11} className="text-blue-200 shrink-0" />
                <span>{pet.matchReason}</span>
              </span>
            </div>
          )}
        </div>
