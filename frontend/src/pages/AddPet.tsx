...
import { ChevronLeft, ImagePlus, UploadCloud, X, Crop as CropIcon, AlertCircle, Plus, FileText, CheckCircle2, ShieldCheck, Trash2, Syringe, Bug, MapPin } from 'lucide-react';
...
export default function AddPet() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!isAuthenticated) {
        navigate('/login', { state: { returnUrl: '/dashboard/add-pet' }, replace: true });
  ...
  useEffect(() => {
    const prefillShelterLocation = async () => {
      try {
        const res = await fetch('/api/refugios/me', { credentials: 'include' });
        if (res.ok) {
          const data = await res.json();
          if (data && data.direccion) {
            setUbicacion(data.direccion);
          } else if (data && data.ubicacion) {
    ...
    if (isAuthenticated && user?.rol === 'REFUGIO') {
      prefillShelterLocation();
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const msg = 'Ocurrió un error al publicar la mascota en el servidor.';
        setError(msg);
        showToast(msg, 'error');
      }
    } catch (err) {
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input 
                      ...
                      className="w-full pl-12 pr-5 py-3.5 rounded-[1rem] bg-gray-50/80 border border-gray-200 focus:bg-white focus:border-[#0B84FF] outline-none transition-all font-medium text-gray-900" 
