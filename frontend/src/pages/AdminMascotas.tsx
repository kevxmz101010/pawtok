  ...
  const toggleEstado = async (id: number, estadoActual: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        handleShowToast(`Estado cambiado a ${nuevoEstado}`, 'success');
      }
    } catch (err) {
      handleShowToast('Error al cambiar estado', 'error');
    }
  };
              ...
              <table className="w-full text-left border-separate border-spacing-y-2">
                <thead className="text-gray-400 text-xs font-bold uppercase px-4">
                  <tr>
                    ...
                    <th className="px-6 py-3">Categoría & Raza</th>
                      ...
                      <tr key={m.id} className="bg-gray-50/60 hover:bg-blue-50/30 transition-colors rounded-2xl group">
                        ...
                        <td className="px-6 py-4">
                          <button
                            ...
                            className={`px-3 py-1 text-xs font-bold rounded-full transition shadow-sm border ${
                              m.estado === 'DISPONIBLE'
