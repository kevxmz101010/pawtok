...
import { ChevronLeft, ChevronRight, MapPin, Home, Syringe, Bug, Calendar, FileText, Check, Plus, X, Trash2, ExternalLink } from 'lucide-react';
import ShelterProfileModal from '../components/ShelterProfileModal';
...
import { formatPetImageUrl, formatCleanText, formatPetAge, formatConNinos } from '../utils/imageUtils';
...
export default function PetDetails() {
  ...
  // Compatibilidad: rescatar matchScore si venimos de la encuesta o de sesión
  const stateMatchScore = location.state?.matchScore as number | undefined;
  const matchScore = useMemo(() => {
    if (user && user.rol !== 'USUARIO') return null;
    if (stateMatchScore) return stateMatchScore;
    if (!pet) return null;
    try {
      const savedUserId = sessionStorage.getItem('pawtok_survey_user_id');
      const expectedId = user ? String(user.id) : 'guest';
      if (savedUserId && savedUserId !== expectedId) return null;

      const saved = sessionStorage.getItem('pawtok_survey');
      if (!saved) return null;
      const survey = JSON.parse(saved);
      const petType = (pet.categoria || '').toLowerCase();
      const requiredType = (survey.tipo || '').toLowerCase();
      if (requiredType && petType !== requiredType) return null;

      const normKids = (pet.conNinos || '').toLowerCase();
      if (survey.tiene_ninos && normKids.includes('no')) return null;

      let score = 65;
      const normTamano = (pet.tamano || '').toLowerCase();
      const normEnergia = (pet.energia || '').toLowerCase();

      if (survey.vivienda === 'apartamento') {
        if (normTamano.includes('pequ') || normTamano.includes('med')) score += 18;
        else if (normTamano.includes('gran')) score -= 25;
        if (normEnergia.includes('baj') || normEnergia.includes('med')) score += 10;
        else if (normEnergia.includes('alt')) score -= 15;
      } else if (survey.vivienda === 'finca') {
        if (normTamano.includes('gran') || normEnergia.includes('alt')) score += 20;
        else score += 10;
      } else {
        score += 15;
      }

      if (survey.tiene_ninos && (normKids.includes('si') || normKids.includes('s'))) score += 15;
      if (survey.tiempo_solo === 'mucho' && petType === 'gato') score += 15;
    ...
    const fetchPet = async () => {
      try {
        ...
        if (user && user.rol === 'USUARIO') {
          try {
            const adRes = await fetch('/api/adopciones/mis-adopciones', { credentials: 'include' });
            if (adRes.ok) {
              const list: any[] = await adRes.json();
              const exists = list.some((a: any) => 
                (a.mascotaId === Number(id) || a.mascota?.id === Number(id)) &&
                String(a.estado).toUpperCase() !== 'RECHAZADA'
              );
              setYaSolicitada(exists);
            }
          } catch (e) { console.error('Error checking existing adoption requests:', e); }
        }
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 overflow-hidden relative">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32 pb-10 min-h-screen flex flex-col items-center">
          ...
          <div className="text-center mt-8 md:mt-10 max-w-lg mx-auto">
            ...
            {matchScore && (
              <div className="mt-2.5 flex justify-center">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full font-bold text-xs sm:text-sm shadow-sm">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-6">
              {pet.raza && (
                <span className="px-3 py-1 text-sm font-semibold text-gray-700 bg-gray-100 border border-gray-200 rounded-full">
                  Raza: {formatCleanText(pet.raza)}
                </span>
              )}
              {pet.edad && (
                <span className="px-3 py-1 text-sm font-semibold text-gray-700 bg-gray-100 border border-gray-200 rounded-full">
                  {formatPetAge(pet.edad)}
                </span>
              )}
              {pet.peso && (
                <span className="px-3 py-1 text-sm font-semibold text-blue-600 bg-blue-50 border border-blue-200 rounded-full">
                  {pet.peso} kg
                </span>
              )}
              {pet.tamano && (
                <span className="px-3 py-1 text-sm font-semibold text-purple-600 bg-purple-50 border border-purple-200 rounded-full">
                  {formatCleanText(pet.tamano)}
                </span>
              )}
              {pet.energia && (
                <span className="px-3 py-1 text-sm font-semibold text-orange-600 bg-orange-50 border border-orange-200 rounded-full">
                  Energía: {formatCleanText(pet.energia)}
                </span>
              )}
              {pet.conNinos && (
                <span className={`px-3 py-1 text-sm font-semibold border rounded-full ${formatConNinos(pet.conNinos) === 'Sí' ? 'text-green-600 bg-green-50 border-green-200' : 'text-red-600 bg-red-50 border-red-200'}`}>
                  Niños: {formatConNinos(pet.conNinos)}
                </span>
              )}
              {pet.personalidad && pet.personalidad.split(',').map((p, i) => {
                ...
                const cleanP = formatCleanText(p.trim());
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 mt-8 pt-6 border-t border-gray-100">
               <div className="flex items-center gap-2 text-gray-600">
                ...
                <button
                   ...
                   className="group inline-flex items-center gap-2 text-gray-700 hover:text-[#0B84FF] bg-blue-50/50 hover:bg-blue-100/70 border border-blue-100/80 px-3.5 py-1.5 rounded-full transition-all cursor-pointer shadow-2xs active:scale-95"
                   title="Ver toda la información del refugio, su horario, mascotas, redes y contacto"
                >
                   <Home className="text-[#0B84FF] w-4 h-4 transition-transform group-hover:scale-110"/> 
                   <span className="font-semibold text-sm md:text-base underline-offset-4 group-hover:underline">
                     {(pet as any).refugioNombre || `Refugio #${pet.idRefugio || 'Pawtok'}`}
                   </span>
                   <ExternalLink className="w-3.5 h-3.5 text-blue-400 group-hover:text-[#0B84FF] ml-0.5" />
                </button>
            </div>
          </div>
        </BlurFade>
        ...
        {/* 4. ACCIÓN DE ADOPCIÓN SEGÚN ESTADO Y ROL */}
        <BlurFade delay={0.45} inView className="mt-8 mb-12 flex justify-center px-4">
          {pet.estado !== 'DISPONIBLE' ? (
            <div className="bg-gray-100 text-gray-600 px-8 py-4 rounded-2xl font-bold text-base border border-gray-200 shadow-sm flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-gray-400"></span>
              Esta mascota no está disponible para adopción ({pet.estado})
            </div>
          ) : user && user.rol === 'REFUGIO' ? (
            <div className="bg-blue-50 border border-blue-200 text-blue-900 px-8 py-3.5 rounded-2xl font-bold text-sm shadow-xs flex items-center justify-center gap-2">
              <span>Estás navegando con perfil de Refugio</span>
            </div>
          ) : user && user.rol === 'ADMIN' ? (
            <div className="bg-purple-50 border border-purple-200 text-purple-900 px-6 py-4 rounded-2xl text-center shadow-sm max-w-lg">
              <p className="font-bold text-sm">Modo Administrador</p>
              <p className="text-xs text-purple-700 mt-1">
                Visualizando ficha de mascota. Las solicitudes de adopción son exclusivas para cuentas de adoptantes.
              </p>
            </div>
          ) : yaSolicitada ? (
            <div className="flex flex-col sm:flex-row items-center gap-3">
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-6 py-3.5 rounded-2xl font-bold text-sm shadow-xs flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
                <span>Ya has enviado una solicitud para {pet.nombre}</span>
              </div>
              <button
                type="button"
                onClick={() => navigate('/cuenta')}
                className="px-6 py-3.5 rounded-2xl bg-gray-900 text-white font-bold text-sm hover:bg-gray-800 active:scale-95 transition shadow-sm cursor-pointer"
              >
                Ver en mi cuenta
              </button>
            </div>
      ...
      {pet && (
        <ShelterProfileModal
          ...
          shelterIdentifier={pet.idRefugio || (pet as any).refugio || (pet as any).refugioNombre}
          fallbackName={(pet as any).refugioNombre || 'Refugio Asociado'}
          fallbackLocation={pet.ubicacion || 'Colombia'}
        />
      )}
